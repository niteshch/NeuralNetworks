...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out,layer_cnt=2,activation=T.tanh):
        """Initialize the parameters for the multilayer perceptron
        ...
        :type layer_cnt: int
        :param layer_cnt: number of hidden layers
        ...
        # Since we are dealing with n hidden layers MLP, this will translate
        # into n HiddenLayer with a activation function connected to the
        # LogisticRegression layer; the activation function can be replaced by
        # sigmoid or any other nonlinear function
        self.hiddenLayer = []
        ...
        for val in range(layer_cnt - 1):
            n = len(self.hiddenLayer)
            prevLayer = self.hiddenLayer[n - 1]
            self.hiddenLayer.append(HiddenLayer(
                rng=rng,
        ...
        # of the hidden layer
        n = len(self.hiddenLayer)
        lastLayer = self.hiddenLayer[n - 1]
        self.logRegressionLayer = LogisticRegression(
            input=lastLayer.output,
            n_in=n_hidden,
            n_out=n_out
        )
        # L1 norm ; one regularization option is to enforce L1 norm to
        # be small
        l1 = abs(self.hiddenLayer[0].W).sum()
        for val in self.hiddenLayer[1:]:
            l1 += abs(val.W).sum()
        self.L1 = (
            l1 + abs(self.logRegressionLayer.W).sum()
        )

        # square of L2 norm ; onelayer_cnt=2,activation=T.tanh regularization option is to enforce
        # square of L2 norm to be small
        l2 = (self.hiddenLayer[0].W ** 2).sum()
        for val in self.hiddenLayer[1:]:
            l2 += abs(val.W ** 2).sum()
        self.L2_sqr = (
            l2 + (self.logRegressionLayer.W ** 2).sum()
        )
        ...
        # the parameters of the model are the parameters of the two layer it is
        # made out of
        tempParams = self.hiddenLayer[0].params
        for val in self.hiddenLayer[1:]:
            tempParams += val.params
        self.params = tempParams + self.logRegressionLayer.params

        # keep track of model input
        self.input = input
        print("Number of Hidden Layers", len(self.hiddenLayer))

# TODO: you might need to modify the interface
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, verbose=False,layer_cnt=2,activation=T.tanh):
    """
